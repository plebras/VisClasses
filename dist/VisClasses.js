...
var VisGrid = class {
  ...
  get b() {
    return this.h - this.pB;
  }
  get l() {
    return this.pL;
  }
  get r() {
    return this.w - this.pR;
  }
  get m() {
    return this.t + this.iH / 2;
  }
  get c() {
    return this.l + this.iW / 2;
  }
  get tL() {
...
function bisector(f) {
  ...
  function left3(a, x, lo = 0, hi = a.length) {
    if (lo < hi) {
      if (compare1(x, x) !== 0)
        return hi;
      do {
  ...
  function right3(a, x, lo = 0, hi = a.length) {
    if (lo < hi) {
      if (compare1(x, x) !== 0)
        return hi;
      do {
...
function tickIncrement(start3, stop, count) {
  var step = (stop - start3) / Math.max(0, count), power = Math.floor(Math.log(step) / Math.LN10), error = step / Math.pow(10, power);
...
function axis(orient, scale) {
  ...
  function axis2(context) {
    ...
    text.attr(x, k * spacing).text(format3);
...
function band() {
  ...
  function rescale() {
    var n = domain().length, reverse = r1 < r0, start3 = reverse ? r1 : r0, stop = reverse ? r0 : r1;
...
// src/utils/vis/Visualisation.js
var Vis = class {
  #svgWidth;
  #svgHeight;
  #svgMargin;
  ...
  constructor({
    ...
    this._svg = select_default2(container).append("svg").classed(classed, true).classed("vis", true).attr("width", this._grid.w).attr("height", this._grid.h);
    this._title = this._svg.append("text").classed("title", true).attr("transform", `translate(${this._grid.w / 2},25)`);
    this.setTitle(title);
  }
  #resize() {
...
function arrow(_ref) {
  ...
  var minProp = axis2 === "y" ? top2 : left2;
  var maxProp = axis2 === "y" ? bottom2 : right2;
  ...
  var offset2 = within(min4, center2, max4);
  var axisProp = axis2;
...
function detectOverflow(state, options) {
  ...
  if (elementContext === popper && offsetData) {
    ...
    Object.keys(overflowOffsets).forEach(function(key) {
      ...
      var axis2 = [top2, bottom2].indexOf(key) >= 0 ? "y" : "x";
...
// src/BarChart.js
var BarChart = class extends Vis {
  #V;
  #K;
  #C;
  #T;
  ...
  constructor({
    ...
    this.#xScale = band().padding(xPadding);
    this.#yScale = yType().nice();
    this.#yDomainC = yDomain;
    this.#barGroup = this._svg.append("g").classed("bars", true).attr("transform", `translate${this._grid.tLS}`);
    ...
    this.#xLabel = this._svg.append("text").classed("legend", true).attr("transform", `translate${this._grid.bCS}`).attr("dy", 30);
    this.#yLabel = this._svg.append("text").classed("legend", true).attr("transform", `translate${this._grid.mLS} rotate(-90)`).attr("dy", -30);
  ...
  #makeTransitions() {
    let duration = 300, ease = linear2;
    let tRemove = transition().ease(ease).duration(duration);
    let tPosition = transition().ease(ease).duration(duration).delay(duration);
    let tSize = transition().ease(ease).duration(duration).delay(duration * 2);
    return { tRemove, tPosition, tSize };
  }
  #updateScales() {
    let yDomain = this.#yDomainC === void 0 ? [Math.min(0, min(this.#data, this.#V)), Math.max(0, max(this.#data, this.#V))] : this.#yDomainC;
    let xDomain = this.#data.map(this.#K);
    this.#xScale.domain(xDomain).range([0, this._grid.iW]);
    this.#yScale.domain(yDomain).range([this._grid.iH, 0]);
  }
  #updateAxes(tPosition, tSize) {
    axisBottom(this.#xScale).tickSize(0).tickPadding(9)(this.#xAxis.transition(tPosition));
    axisLeft(this.#yScale).ticks(this.#yTicks, this.#yFormat)(this.#yAxis.transition(tSize));
    this.#xAxis.transition(tSize).attr("transform", `translate(${this._grid.l},${this._grid.b - this._grid.iH + this.#yScale(0)})`);
    this.#xAxis.selectAll("g.tick").filter((d) => this.#data.filter((d2) => this.#K(d2) == d).length > 0 ? this.#V(this.#data.filter((d2) => this.#K(d2) == d)[0]) < 0 : false).selectAll("text").transition(tSize).attr("y", -12);
  }
  #updateBars(tRemove, tPosition, tSize) {
    let yOrigin = this.#yScale(Math.max(0, this.#yScale.domain()[0]));
    this.#barGroup.selectAll("rect.bar").data(this.#data, this.#K).join(
      (enter) => enter.append("rect").classed("bar", true).attr("y", yOrigin),
      (update) => update,
      (exit) => exit.call((exit2) => exit2.transition(tRemove).attr("height", 0).attr("y", yOrigin).remove())
    ).transition(tPosition).attr("x", (d) => this.#xScale(this.#K(d))).attr("width", this.#xScale.bandwidth()).transition(tSize).style("fill", this.#C).attr("height", (d) => Math.abs(yOrigin - this.#yScale(this.#V(d)))).attr("y", (d) => Math.min(yOrigin, this.#yScale(this.#V(d))));
    this.#setTooltips();
    this.#setEvents();
  }
  #setColor() {
    this.#barGroup.selectAll("rect.bar").transition().style("fill", this.#C);
  }
  #setTooltips() {
    this.#tips.forEach((t) => t.destroy());
    if (this.#T !== void 0) {
      let bars = this.#barGroup.selectAll("rect.bar").attr("data-tippy-content", this.#T);
      this.#tips = tippy_esm_default(bars.nodes(), { allowHTML: true, duration: 50, theme: "vis-dark" });
    } else {
      this.#tips = [];
    }
  }
  #setEvents() {
    this.#barGroup.selectAll("rect.bar").on("mouseover", (e, d) => {
      this.#over(e, d);
    }).on("mouseout", (e, d) => {
      this.#out(e, d);
    }).on("click", (e, d) => {
  ...
  #select(keys, type2) {
    this.#barGroup.selectAll("rect.bar").classed(type2, false).filter((d) => keys.includes(this.#K(d))).classed(type2, true);
    return this;
  }
  render(dataset) {
    this.#data = dataset;
  ...
  setYType(type2 = linear3) {
    this.#yScale = type2().nice();
    this.#refresh();
    return this;
  }
  setYDomain(domain = void 0) {
    this.#yDomainC = domain;
    this.#refresh();
    return this;
  }
  setYTicks(ticks2 = void 0) {
    this.#yTicks = ticks2;
    this.#refresh();
    return this;
  }
  setYFormat(format3 = void 0) {
    this.#yFormat = format3;
    this.#refresh();
    return this;
  }
  setOver(f = () => {
  ...
  selectBars(keys = [], type2 = "selected") {
    this.#select(keys, type2);
    return this;
  }
};
